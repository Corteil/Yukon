...
def _serialise(state, cam_rotation=0, aruco_enabled=None,
    ...
    return {
        ...
        'cam_fl_cap': state.cam_front_left_cap,
        'cam_fr_cap': state.cam_front_right_cap,
        'cam_re_cap': state.cam_rear_cap,
        ...
        'telemetry': {
            ...
            'rr_temp':    round(t.rr_temp,   1),
            'fr_temp':    round(t.fr_temp,   1),
            'fl_temp':    round(t.fl_temp,   1),
            'rl_temp':    round(t.rl_temp,   1),
            'rr_current': round(t.rr_current, 2),
            'fr_current': round(t.fr_current, 2),
            'fl_current': round(t.fl_current, 2),
            'rl_current': round(t.rl_current, 2),
            'rr_fault':   t.rr_fault,
            'fr_fault':   t.fr_fault,
            'fl_fault':   t.fl_fault,
            'rl_fault':   t.rl_fault,
...
.cam-lens-cap{position:absolute;bottom:8px;left:50%;transform:translateX(-50%);
  font-size:11px;font-weight:bold;color:#000;background:#ffcc00;
  padding:2px 8px;border-radius:3px;display:none;pointer-events:none}
...
/* Motor module grid */
.mod-grid{display:grid;grid-template-columns:1fr 1fr;gap:4px;padding:4px 8px}
.mod-cell{background:var(--bg);border:1px solid var(--border);border-radius:4px;padding:4px 6px;transition:border-color .3s}
.mod-cell.fault{border-color:var(--red);background:#1a0808}
.mod-name{font-size:9px;color:var(--gray);letter-spacing:.5px;margin-bottom:2px;font-weight:bold}
.mod-vals{display:flex;gap:6px;font-size:11px;margin-bottom:2px}
.mod-tmp{color:var(--yellow)}
.mod-amp{color:var(--orange)}
.mod-status{font-size:10px;font-weight:bold}
.mod-status.ok{color:var(--green)}
  ...
  <div id="mob-tabs">
    ...
    <div class="mob-page active" id="mob-drive">
      <div class="cam-wrap" id="mob-cam-wrap" style="max-height:45vh">
        ...
        <div class="cam-lens-cap" id="mob-cam-lens-cap">LENS CAP ON</div>
      </div>
    ...
    <div class="mob-page" id="mob-telem">
      <div class="mob-section">
        ...
        <div class="mod-grid">
          <div class="mod-cell" id="mob-mod-fl">
            <div class="mod-name">FL·S3</div>
            <div class="mod-vals"><span class="mod-tmp" id="mob-fl-temp">--</span><span class="mod-amp" id="mob-fl-curr">--</span></div>
            <span class="mod-status ok" id="mob-fl-fault">OK</span>
          </div>
          <div class="mod-cell" id="mob-mod-fr">
            <div class="mod-name">FR·S2</div>
            <div class="mod-vals"><span class="mod-tmp" id="mob-fr-temp">--</span><span class="mod-amp" id="mob-fr-curr">--</span></div>
            <span class="mod-status ok" id="mob-fr-fault">OK</span>
          </div>
          <div class="mod-cell" id="mob-mod-rl">
            <div class="mod-name">RL·S4</div>
            <div class="mod-vals"><span class="mod-tmp" id="mob-rl-temp">--</span><span class="mod-amp" id="mob-rl-curr">--</span></div>
            <span class="mod-status ok" id="mob-rl-fault">OK</span>
          </div>
          <div class="mod-cell" id="mob-mod-rr">
            <div class="mod-name">RR·S1</div>
            <div class="mod-vals"><span class="mod-tmp" id="mob-rr-temp">--</span><span class="mod-amp" id="mob-rr-curr">--</span></div>
            <span class="mod-status ok" id="mob-rr-fault">OK</span>
          </div>
        </div>
        <div class="tgrid">
          <div class="field">
...
function htmlCamera(i, camKey) {
  ...
  return `
    <div class="cam-wrap">
      ...
      <div class="cam-lens-cap" id="q${i}-cam-lens-cap">LENS CAP ON</div>
    </div>`;
}
...
function htmlTelemetry(i) {
  ...
  const modCells = mods.map(([k, lbl]) => `
    <div class="mod-cell" id="q${i}-mod-${k}">
      <div class="mod-name">${lbl}</div>
      <div class="mod-vals">
        <span class="mod-tmp" id="q${i}-${k}-temp">--</span>
        <span class="mod-amp" id="q${i}-${k}-curr">--</span>
      </div>
      <span class="mod-status ok" id="q${i}-${k}-fault">OK</span>
    </div>`).join('');
  return `
    <div class="tgrid three">
    ...
    <div class="tgrid" style="padding-top:0">
      <div class="field"><label>Board</label><span id="q${i}-board">--</span></div>
    </div>
    <div class="mod-grid">${modCells}</div>
...
function updateCameraPanel(i, camKey, s) {
  ...
  const capKey = camKey==='front_left'?'cam_fl_cap': camKey==='front_right'?'cam_fr_cap':'cam_re_cap';
  const ok  = !!s[okKey];
  const rec = !!s[recKey];
  const cap = !!s[capKey];
  ...
  const capEl = el(`q${i}-cam-lens-cap`);
  if (capEl) capEl.style.display = (ok && cap) ? 'block' : 'none';
...
function updateTelemPanel(i, s) {
  ...
  for (const [k, fk, tk, ak] of [
    ['fl','fl_fault','fl_temp','fl_current'],
    ['fr','fr_fault','fr_temp','fr_current'],
    ['rl','rl_fault','rl_temp','rl_current'],
    ['rr','rr_fault','rr_temp','rr_current'],
  ]) {
    const f = t[fk]; const cell = el(`q${i}-mod-${k}`);
    if (cell) cell.className = 'mod-cell' + (f ? ' fault' : '');
    const fs = el(`q${i}-${k}-fault`);
    if (fs) { fs.textContent = f ? 'FAULT' : 'OK'; fs.className = 'mod-status '+(f?'err':'ok'); }
    se(`q${i}-${k}-temp`, t[tk]!=null ? t[tk].toFixed(0)+'°C' : '--');
    se(`q${i}-${k}-curr`, t[ak]!=null ? t[ak].toFixed(1)+' A' : '--');
  }
}
...
function updateMobile(s) {
  ...
  const mlc=el('mob-cam-lens-cap'); if(mlc) mlc.style.display=(ok&&s.cam_fl_cap)?'block':'none';
  ...
  for (const [k, fk, tk, ak] of [
    ...
    const f = t[fk]; const cell = el(`mob-mod-${k}`);
    if (cell) cell.className = 'mod-cell' + (f ? ' fault' : '');
    const fs = el(`mob-${k}-fault`);
    if (fs) { fs.textContent = f ? 'FAULT' : 'OK'; fs.className = 'mod-status '+(f?'err':'ok'); }
    se(`mob-${k}-temp`, t[tk]!=null ? t[tk].toFixed(0)+'°C' : '--');
    se(`mob-${k}-curr`, t[ak]!=null ? t[ak].toFixed(1)+' A' : '--');
  }
...
function reloadCameraStreams() {
  ...
  // Called on SSE reconnect — the server restart that broke the SSE also
  // terminated the MJPEG connections, so the <img> elements need a nudge.
  for (let i = 0; i < 4; i++) {
    if (streamPaused[i]) continue;  // user deliberately paused — leave alone
    const type = quadTypes[i];
    if (!type || !type.startsWith('camera:')) continue;
    const camKey = type.split(':')[1];
    const img = el(`q${i}-cam-img`);
    if (!img) continue;
    // Clear then restore src — gives the browser a fresh MJPEG request
    img.src = '';
    img.src = '/stream/' + camKey;
  }
  // Mobile camera image
  const mi = el('mob-cam-img');
  if (mi && mi.src && mi.src.includes('/stream/')) {
    const saved = mi.src;
    mi.src = '';
    mi.src = saved;
  }
