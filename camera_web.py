...
from robot.camera_controls import (
    CAPTURE_SIZES, GAINS, EXP_STEPS, ARUCO_DICTS,
...
class CameraState:
    def __init__(self):
        ...
        # calibration
        self._calib      = CalibrationMaps(CALIB_FILE)
    ...
    def calib_available(self):
        return self._calib.available

    def get_cal_maps(self, w, h):
        return self._calib.get_maps(w, h)
...
def camera_worker(state: CameraState):
    ...
    while True:
        ...
        if state.show_aruco:
            ...
            _draw_aruco_on_frame(annotated, aruco_st)
        else:
