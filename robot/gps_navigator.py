...
class GpsNavigator:
    ...
    def load_waypoints(self, source):
        ...
        if isinstance(source, str):
            with open(source) as f:
                data = json.load(f)
            self._waypoints = [Waypoint.from_dict(d) for d in data]
    ...
    def update(
        ...
        if has_fix and gps.timestamp != self._last_gps_ts:
            ...
            # Update IMU target from GPS bearing
...
def save_waypoints(waypoints: List[Waypoint], path: str):
    """Save a list of Waypoint objects to a JSON file."""
    data = [{"lat": wp.lat, "lon": wp.lon, "label": wp.label}
            for wp in waypoints]
